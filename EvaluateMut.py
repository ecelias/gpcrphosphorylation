...
def score_vars(df):
    ...
    for index, row in df.iterrows():
        ...
        elif var_pos == current_psite and mut_res not in {'Y', 'S', 'D', 'E', 'T'} and psite_residue.upper() in {'S','T'}:
        ...
        if wt_scores is not None:
            k = str(current_id) + ',' + wt_seq + ',' + str(var_pos) + ',' + wt_res + ',' + mut_res + ',' + str(
                current_psite) + ',' + psite_residue
...
def score_df(dictionary):
    ...
    df = df.reset_index()
    df[['ACC_ID', 'WT_SEQUENCE', 'VARIANT_POSITION', 'WT_RES', 'MUT_RES', 'PHOSPHOSITE', 'PHOSPHOSITE_RESIDUE']] = df[
...
#test = y_score_dict_mut('EALKFYTDPS')
#print(test)
#test_df = pm.test_vars
##test = score_vars(test_df)
#test2 = score_df(test)

# cosmic_scores_dict = score_vars(Aim1.cosmic_phos_muts)
# cosmic_scores = score_df(cosmic_scores_dict)
# Aim1.to_csv(cosmic_scores, 'cosmic_kinase_scores')

#cosmic_sample_scores_dict = score_vars(Aim1.PM_cosmic_sample)
#cosmic_sample_scores = score_df(cosmic_sample_scores_dict)
#Aim1.to_csv(cosmic_sample_scores, 'cosmic_sample_kinase_scores')
